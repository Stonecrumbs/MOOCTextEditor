	...
	protected int countSyllables(String word)
	{
		...
		if (word.isEmpty())
			...
			for (int i = 0; i < word.length() - 1; i++)
				if (("AEIOUYaeiouy".indexOf(word.charAt(i)) != -1) && ("AEIOUYaeiouy".indexOf(word.charAt(i + 1)) == -1))
					numSyllables++;

			if (("Ee".indexOf(word.charAt(word.length() - 1)) != -1) && ("AEIOUYaeiouy".indexOf(word.charAt(word.length() - 2)) != -1))
				numSyllables++;

			if ("AIOUYaiouy".indexOf(word.charAt(word.length() - 1)) != -1)
				numSyllables++;
		}
